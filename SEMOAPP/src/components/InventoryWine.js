...
const InventoryWine = ({ score, notes, aroma, wineNum, adv, sugarRating, acidRating, bodyRating, tanninRating, alcoholRating, title, people, style, tasks, children, ...props }) => {
var advList;
...
return (
  ...
  <MaterialCommunityIcons name="bottle-wine" color={theme.colors.darkBlue} size={30}/>
    ...
    <View style={styles.textContainerVert}>
      
    <Text style={styles.aroma}>Aroma: {aroma}</Text>
    <Text style={styles.aroma}>Score: {score}</Text>
    <View style={styles.barContainer}>
    <View style={styles.barBackground}>
      <View style={styles.barF}>
        <Text></Text>
        </View>
      </View></View>
    
    <Text style={styles.aroma}>Notes: {notes}</Text>
    </View>
    <View style={styles.buttonContainer}><Button style={styles.join} mode="outlined" onPress={openWindow}>
      <Text style={styles.join} >Update</Text>
...
export default memo(InventoryWine);
